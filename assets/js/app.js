...
function InicializarFire() {
  ...
  firebase.auth().onAuthStateChanged(function(user){
    ...
    if (user) {
      ...
      $('.cont-myperfil').removeClass('hidden')
      $('.user-info').append('<img class="user-information img-responsive" src ='+userPhoto+' >')
      $('.info-user').append('<h2>' + displayName + '</h2>'
        + '<h4>' + email + '</h4>')
...
function getMovies(movie) {
  ...
  var infoFav = firebase.database().ref('Users');
  ...
  function showMovie(info) {
    if (info.Response === 'false') {
      ...
      if(!user){
        $('#show').append(`
          <div class='row'>
            <div class="col-md-3 interaction">
              <img src="${info.Poster}">
      ...
      `);
        ...
        $('#show').append(`
          <div class='row'>
            <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 interaction">
              <img src="${info.Poster}">
              <div class="row">
                ...
                <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
                  <button class="btn btn-default btn-fav" type="button" name="button">Add Favourite</button>
                </div>
              </div>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-7 col-md-offset-1 col-lg-7 col-lg-offset-1">
              <h5>Title:</h5> <h3>${info.Title}</h3>
              <h5>Argumento:</h5> <h4>${info.Plot}</h4>
              <h5>Director:</h5> <h4>${info.Director}</h4>
              <h5 class='cont-afterhide'>Producción:</h5> <h4 class='cont-afterhide'>${info.Production}</h4>
  ...
  $('.my-perfil').click(function() {
    ...
    infoFav.orderByChild('userID').on('value', function(snapshot){
      snapshot.forEach(function(e){
        var info = e.val();
        $('.user-search').append('<div class="title-movie thumbnail card"><img class="img-mov img-responsive" src="' 
          + info.Imagen + '"><p class="txt_movie">' + info.Title + '</div>');
      })
    })
